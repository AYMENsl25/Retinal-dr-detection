...
export default function AnalyzePage() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const backendPanels = response.panels
      const hasBackendPanels = Boolean(backendPanels.mask && backendPanels.heatmap && backendPanels.overlay)
      setPanels(hasBackendPanels ? (backendPanels as PanelImages) : panelImages)
      setResult(response)
  ...
  function handleExportPdf() {
    ...
    if (result) {
      document.title = `RetinaScope-AI Case ${result.case_id.slice(0, 8)}`
    }
    window.print()
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SiteHeader patientId={result?.case_id} onExportPdf={result ? handleExportPdf : undefined} />

      <main className="mx-auto max-w-7xl px-4 md:px-6 py-6 space-y-6 print:max-w-none print:px-0">
